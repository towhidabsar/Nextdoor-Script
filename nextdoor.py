...
def parse_search(search_term, get_replies=False):
	# Load ability to use .env
	load_dotenv()
	...
	# Search for a specific term
	search = driver.find_element(by=By.XPATH, value='//input[@id="search-input-field"]')
	...
	driver.find_element(By.XPATH, value='.//a[@data-testid="tab-posts"]').click()
	# Use Selenium to scroll 'range' number of times
	# Change the second number in 'range(x, y)' to determine how many times you want it to scroll down.
	for i in range(1, 20):
		...
		number_of_posts = driver.find_elements(by=By.XPATH, value='.//div[@class="css-15luflj"]')
	...
	# postNodes = driver.find_elements(by=By.XPATH, value='//a[@class="uUGmI2_t css-1q9s7yp"]')
	# print("postNodes")
	# print(postNodes)
	# Iterate over each post node that has an author to get data in an organized fashion
	posts = pd.DataFrame()
	...
	for url in all_posts:
		...
		# 	})
		# ])
		posts.to_json(f'{search_term}_posts.jsonl', orient='records', lines=True)
		# replies.to_json('replies.jsonl', orient='records', lines=True)
		time.sleep(2)
	driver.quit()

parse_search('homeless')